    ...
    <div className="grid grid-2">
      <div className="card">
        ...
        <table>
          <tbody>
            ...
            <tr>
              <td>Öncelik</td>
              <td>
                <span className={`badge-pill ${slaPillClass(item.priority, item.slaBreached)}`}>
            ...
            <tr>
              <td>SLA</td>
              <td>{formatRemaining(item.slaDueAt, item.completedAt)}</td>
            </tr>
            <tr>
              <td>Dönüşüm Olasılığı</td>
              <td>{item.conversionProbability != null ? `${Math.round(item.conversionProbability * 100)}%` : '-'}</td>
            </tr>
        ...
        <div style={{ marginTop: 16 }}>
          <label style={{ fontSize: '0.85rem', color: 'var(--color-muted)', fontWeight: 600 }}>Segmenti Değiştir (AI override)</label>
          <select
            style={{ display: 'block', marginTop: 6 }}
            value={item.segment}
            disabled={busy}
            onChange={(e) =>
              runAction(() => updateSegment(item.id, e.target.value as SegmentType), 'Segment güncellendi')
            }
          >
